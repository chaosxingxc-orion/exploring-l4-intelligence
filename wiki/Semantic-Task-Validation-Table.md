...
| Dataset | 验证集 (split · n) | Verifiable reward | Serves | 验证目标 | 验证方案 | Status | Result |
|---|---|---|---|---|---|---|---|
| **mmau-mini** | n=150 (E3, E4); full `test_mini` 1,000 | MCQ accuracy | **CP-1(SQA)**, **CP-3(ρ)**, CP-8, CP-5 | H_prompt−H_fix on MCQ (E3); ρ realization (E4) | E3: 8 task-def × 1 vs 1 fixed × 8, budget 8. E4: modern label-free selectors on the SAME slice's pools | ✅ E3 + E4 done | **E3:** greedy=majority **0.640**; **H_fix = +0.133** (oracle 0.773 — support REAL) but **H_prompt−H_fix = +0.020** (CI [−0.02,+0.06], n.s.), b2 vs generic +0.027 (n.s.). **E4 (ρ realization):** greedy 0.633, oracle 0.773 (**headroom +0.140**); self-certainty **ρ=0.0**, majority/conf-vote **ρ=−0.047**, LLM-judge **ρ=0.143 n.s.** (CI [−0.013,+0.067]) — **no label-free selector significantly beats majority**. `[directional, n=150]`. **Read: (a) support REAL, (b) prompt ≈ nil, (c) realization ≈ nil across modern selectors (confidently-wrong / large-τ; C4).** `_repro/cp1_sqa_hprompt_mmau.json`, `_repro/cp3_selector_realization_mmau.json` |
