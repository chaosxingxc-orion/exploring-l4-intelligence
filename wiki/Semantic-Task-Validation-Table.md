...
| Dataset | 验证集 (split · n) | Verifiable reward | Serves | 验证目标 | 验证方案 | Status | Result |
|---|---|---|---|---|---|---|---|
| **mmau-mini** | n=150 (E3); full `test_mini` 1,000 | MCQ accuracy | **CP-1(SQA)**, CP-8, CP-5 | H_prompt−H_fix on MCQ | E3 (done): 8 task-def × 1 vs 1 fixed × 8, budget 8 | ✅ E3 done | greedy=majority **0.640** (non-saturated); **H_fix = +0.133** (oracle 0.773 — support REAL) but **H_prompt−H_fix = +0.020** (CI [−0.02,+0.06], n.s.) and b2 vs generic +0.027 (n.s.); **majority realizes 0% of the +0.133 headroom (ρ≈0)**. `[directional, n=150]`. **Read: headroom exists (sampling/a), but prompt (b) and label-free selection (c) both ≈ nil.** `_repro/cp1_sqa_hprompt_mmau.json` |
