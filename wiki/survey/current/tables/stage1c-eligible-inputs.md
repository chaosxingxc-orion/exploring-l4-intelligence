---
artifact_id: "SF-STAGE1C-ELIGIBLE-INPUTS-V3-2026-07-22"
...
9-work known-prior reconciliation and 39-row speech/omni supplement have different denominators;
...
| feasible replacement data / nearest prior / evaluator | Select a locally feasible task only in Stage-1C. The control-basis table separates 9 orchestration-only, 9 state/event-gated and 7 evaluator/verifier-gated direct paths; none is currently classified reward-guided selection. Nearest priors must be chosen by task fit, with VRR-Stop as the noise-theory comparator. Required evaluators: true task validity, verifier discrimination, repair damage, decision margin, rounds and latency/cost. |
...
| system-level direct evidence | The retained portfolio contains 43 instruments. The strict speech supplement adds explicit routes for VoiceAgentBench (`DP-2510.07978`), tau-Voice (`DP-2603.13686`), Full-Duplex-Bench v3 (`DP-2604.04847`), Audio2Tool (`DP-2604.22821`), Omni-DeepSearch (`DP-2605.08762`), EVA-Bench (`DP-2605.13841`), IHBench (`DP-2606.19595`), the LALM audio-judge reliability study (`DP-2607.07985`), WavReward (`DP-2505.09558`), SDiaReward (`DP-2603.14889`) and GSRM (`DP-2602.13891`). |
...
| strongest contradiction | Some deterministic task environments already expose terminal success; a well-specified local evaluator may make a generic reward-reliability problem unnecessary. WavReward, SDiaReward and GSRM are trained evaluator boundaries rather than TF-Strict controllers. The audio-judge study also shows that an audio-native evaluator cannot be assumed reliable merely because it accepts raw audio. |
...
| feasible replacement data / nearest prior / evaluator | Candidate instruments have exact availability states in the acquisition matrix: five named datasets are local pinned/baseline-locked, while the LALM-judge recordings are private/pending. Nearest diagnostics include the three speech reward boundaries, Oracle Gap, HALLMARK and VRR-Stop. Measure calibration, MCC/ranking agreement, false positive/negative rates, harm and selection utility; choose a concrete instrument only after Stage-1C comparison. |
