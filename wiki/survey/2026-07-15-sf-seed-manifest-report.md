...
> **HISTORICAL_SUPERSEDED 范围标注（C4A P0-R1,2026-07-16）**：本件审计的是**快照 51 条**的
> 生成与去重；其后 manifest 经增量批次1(9)/批次2(14)/批次3(13,C4-6)/批次4(5,C4A) 增长，
> **现行计数正典 = manifest 逐行枚举 + `sf_package_summary.py` 机器重数**（当前 92）。本件
> 中的「51 条」为快照口径，不再出现在任何 active 签署清单；原文按 append-only 保留不改写。
