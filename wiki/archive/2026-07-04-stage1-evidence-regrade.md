...
| C1 ASR oracle headroom +0.0418 [0.0289, 0.0564] | conclusion | **scoped** | real, but one model / one condition (**+5 dB SNR** — mild additive noise; the code's `snr_db=5` gives noise power = signal/3.16 = +5 dB, corrected from the ambiguous "SNR-5" that read as −5 dB; D5b review SC-5) / one fixed instruction — i.e. it measures H_fix, not H_prompt |
