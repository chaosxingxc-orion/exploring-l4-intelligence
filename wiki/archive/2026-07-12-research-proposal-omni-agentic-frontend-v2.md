...
status: "DRAFT v2.1 — 两轮 owner 修正并入（系统对象/效果裁判 + 基线反转/闭源兼容层/打样逻辑）；未签署，签署前零实验"
...
**第二轮 owner 修正（同日）**：④基线哲学反转——不得要求先把核心模型调到极致再测知识系统；
真实价值场景 = **把闭源 API 搭成可验证、可闭环的系统，针对性提升外挂能力（知识/记忆/技能）**。
基线 = 裸核心标准用法；long-context 塞入/own-ASR 级联从基线改编入**系统复杂度阶梯**（外挂系统的
最低阶形态）；⑤**闭源 API 兼容层为硬约束**：主配置只用黑盒接口（音频/文本进、文本出、采样），
logprob/logit_bias/GBNF 等白盒特权降为白盒扩展层单独申报；⑥本地冻结核心 = 科学载具（可复现），
闭源可迁移性 = 价值主张（接口最小化设计 + 跨核心迁移冒烟作证）。

## 1. 主问题（唯一，效果口径）

> 以冻结 Qwen3-Omni-30B 为核心（科学载具；接口按闭源 API 兼容层约束）的 omni agentic
> system，加装前端多模态知识子系统（检索-发现-使用三段，组件异构、全部冻结使用），能否在
> 知识依赖型语音任务上，对**裸核心标准用法基线**取得 **≥10% 相对**、全家族校正后统计可靠、
> 边界清白的业务效果提升？并在**系统复杂度阶梯**上，精心组织的 RDU 配置相对平凡知识供给
> （long-context 塞入、own-ASR 朴素级联）是否有可靠净增益（组织智慧的次级主张）？
...
**闭源 API 兼容分层**：黑盒层（主配置）= 音频/文本输入、文本输出、多次采样——发现段触发用
输出侧信号（采样自一致性/验证器一致度/答案级置信引出）；白盒扩展层（单独申报）= logprob 触发、
logit_bias/GBNF 注入（本地核心可用，闭源 API 不保证）。已验 logprob 信号改标 white-box tier。
...
**基线** = 裸核心标准用法（标准 prompt，dev 轻调后冻结）。**系统复杂度阶梯**（臂族内，
自低向高）：①long-context 全塞入（平凡供给）；②own-ASR 朴素级联；③RDU 完整配置——主主张
比对基线，阶梯主张比对①②。closed-book 锚点改为
...
- **家族全枚举**：每个假设一个预声明家族（H-sys：最优配置 vs 裸基线 1 + 阶梯对比 2 = 3 比较；S1：3 键空间
...
Week 3：owner 签 Phase-B → custodian 信标抽取 → 单通道确证 → H-sys 判定 + **跨核心迁移
冒烟**（黑盒层配置在第二核心上 1-2 集轻验，作可迁移性证据；候选：MERaLiON-2 或一个闭源 API）。

## 11. Owner / Reviewer 签字位

1. §1 主问题措辞（裸基线 + 阶梯双主张结构）与闭源兼容分层；2. 10% 相对门槛与 per-dataset SESOI 表；3. §3 次级科学点
