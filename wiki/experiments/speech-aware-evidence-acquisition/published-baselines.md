...
## 可比性误差预算（2026-08-09，实测优先；引用本页任何 ours 数字前必读）

把我们的 WER 与已发表数字并列时，**误差不是 ±2%，而是下表各项之和**；其中最大的是抽样，最容易被忽略的是归一化口径。

| 项 | 量级 | 性质 | 依据 |
|---|---|---|---|
| 抽样（E-001 用 10/125 通话） | **±2.5pp** | 随机，最大项 | 逐通话 WER 方差实算（SD≈3.6，SEM≈1.14） |
| 数字归一化口径 | **1.3–1.8pp** | 系统性，对我们不利 | 实测：同一批输出改用数字展开重评分，语料级 WER 26.70%→24.93%（展开为词）/25.42%（折叠为数字） |
| 聚合方式（宏 vs 微平均） | 0.04pp | 可忽略 | 实算：26.74% vs 26.70% |
| fstalign 同义变换 | 未测，估 <0.5pp | 系统性 | Earnings-21 论文称影响约 0.3% 的分歧 token |
| 语料差异（E-21 vs E-22） | 未量化 | 对我们不利 | Earnings-22 的立论即"带口音更难" |
| 系统类别（专用 ASR vs 冻结 omni + 智能体循环） | 不可量化 | — | — |

**根因**：本仓冻结的 `normalize_text_v1` 明确不做数字展开（"no number expansion"），而 Earnings-21/22 论文用的 fstalign 有文本归一化变换（影响约 5% 参考 token，典型如 `2021 ↔ twenty twenty one`）。财报域数字密集且我们的核会口语化数字，因此冻结指标会把**写法差异**记成识别错误。

**使用规则**：(1) 本研究的结论建立在**同模型同协议的配对臂对比**上，那里只受抽样限制；(2) 与已发表数字的比较须声明上表；(3) 需要数值可比时，优先用下面的**同指标校准**而非纸面数字。

### 同指标校准：用本仓冻结评分器重评已发表系统（2026-08-09）

Earnings-21 的发行包在本地带有各系统的**原始转写输出**（`earnings21/output/{amazon,google,microsoft,rev,speechmatics,kaldi_org}`，`.nlp` 格式，与参考同构）。用**本仓冻结的 `normalize_text_v1` + 对齐**重评它们，在 R4 迷你样本的同 10 个通话上：

| 系统 | 论文值（fstalign，全 44 通话） | 本仓指标（同 10 通话） | 差异 |
|---|---|---|---|
...
（Rev 与 Kaldi 未纳入：Rev 的目录结构与其余不同、Kaldi 是 `.ctm` 格式；4 个系统已足够校准，按"验证环节取大致结论"的方针不再扩展。）

**结论**：本仓指标复现已发表数字的偏差在 **±1.3–2.2pp**（其中同时含 10/44 抽样与归一化两个来源）。因此本仓的 E-21 数字**可以与已发表表格并列**，只需声明 ±2pp 的工具口径容差；这比"仅可序数比较"强得多。**同样本同指标的目标区间：15.70%（Amazon，最优）– 17.95%（Microsoft）。** R4 的 no-context 臂产出可直接落入该区间比较。
