...
| **W1** | `speech-mllm-training-free-rl` | 🟢 Mature · **primary study · Stage-1A closeout-preparation (survey-ready gate)** | **System-first canon (S0 signed 续48; supersedes the 续34/35 selector-object framing in place):** primary program = black-box omni agentic system × TF-Strict reward-guided external control; selector line demoted to component dossier (ρ/H/regret = metrics, not the object). **Now (续63, 2026-07-17): Gate S1 correction #4B delivered → P0-R9 narrow re-review pending** — P0-R8 re-review's 3 MAJOR + 2 MINOR all independently reproduced then remediated: fail-closed gate v2 (canon-pinned 92 seeds/61 queries/50 routes/26 sentinels/5 held-out; producer-rerun byte-compare; mutation harness 10/10), record validator V1–V15 + 25-negative fixture set, SF-L12/L13 mirror lanes (owner rulings: agent-era≥2025 held-outs, CV/learning method domains), raw Atom + fulltext provenance, routes v3 self-caught correction, exit mechanism E1–E3 (amendment-7, execution-phase); citation calibration PREDICTION_CONFIRMED. Bundle @9b1f00b/ecd965f, merged to master @67cb2de; **zero queries executed** (attestation). **Stage-1B NOT authorized; M2 frozen.** M1 engineering base at `20d45a8`; genuine best-of-N result (frozen Qwen3-Omni-30B via llama.cpp) retained. |
