...
| **W1** | `speech-mllm-training-free-rl` | 🟢 Mature · **primary study · Stage-1A closeout-preparation (survey-ready gate)** | **System-first canon (S0 signed 续48; supersedes the 续34/35 selector-object framing in place):** primary program = black-box omni agentic system × TF-Strict reward-guided external control; selector line demoted to component dossier (ρ/H/regret = metrics, not the object). **Now (续59, 2026-07-16): Gate S1 search-design = WITHHOLD → amendment-3 remediation batch landed** (arXiv-primary + free-OA rescue source policy; tier-as-prior + study_quality axes; 74 seeds; 51 compiled queries; 50 T1 proceedings routes; REC-1..7 schema propagation) — hostile loop converged + bundle correction #3 pinned @37da7f3; **owner approved (07-16) → narrow re-review application filed (续60)**; **zero queries executed** (attestation). **Stage-1B NOT authorized; M2 frozen.** M1 engineering base at `20d45a8`; genuine best-of-N result (frozen Qwen3-Omni-30B via llama.cpp) retained. |
