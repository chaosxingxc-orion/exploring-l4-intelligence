...
date: 2026-07-15
dating_note: "本批工件按 2026-07-15 系列命名；实际提交时钟为 2026-07-14 晚（+0800）——命名与提交时间差如实登记，不作回溯调整"
...
number_reconciliation: "对账范围有界定：文中标注 CL2 行号的数字已由协调者逐条机器对账 claim-ledger-v2；未入台账的集外数字（如 2606.02981 的 Spearman ρ≈0.90）一律标 AS_GIVEN_BY_REVIEW 不外推；对账中纠正草稿一处错误（KIT ST 兑现实为小幅为正，见 §5.1）"
generated_by: "Claude Fable 5 主会话协调者亲笔（非委托）；素材=三轮博导评审整改后的正典工件（census v2 / ledger v2 / 冻结合同+修正案 №1 / 预注册协议 / 诚信核查 C1/C4 census / P0-R8 机检）"
verified_by: "机械层=P0-R8 校验器 exit 0 + 协调者数字对账；敌意内审环=提交前执行（结果见 frontmatter hostile_review 行）；科学双审/独立盲审 = 待 E2（如实声明）"
hostile_review: "三轮敌意内审环（R1 预检：初稿 8 缺陷+2 警示，最重=0 字节验收工件引用——已重建入库并在 §3.3/§10 双锚披露；R2 三镜头委托复审〔事实数字/术语token/矛盾覆盖〕：28 项发现，全部数字与哈希核验为真，缺陷集中在清单构成、冻结判据忠实度、0 字节锚未在正文披露、完成度宣称过早——协调者逐项亲验后全部修复；R3 收敛复检：**CONVERGED**——11 组修复全部核验 FIXED、零新发现〔3 项 trivial 润色已采纳〕，R3 代理独立复跑 P0-R8 与 ledger/census 机器重数全部复现）；P0-R8 机器门复跑工件 = docs/checks/2026-07-15-p0r8-gate-run-proposal-commit.txt"
...
> 限定保留为候选、我们对自己纠了哪些错、以及下一步以何种可证伪判据继续。它不主张任何已证结论。
...
- **现状一句话**：五个候选身份（I1/I2/I3/I4/UMBRELLA；I2 拆宽式 bare-I2 与合取 strict-I2 两式，
  故合同为六份）中，I1 已被直接占据（kill 方向保持）、bare-I2 机制级被占、
  strict-I2/I3-combined/UMBRELLA 在保留记录集中无直接匹配（均带强制伴随 token：检索宇宙缺失/
  饱和不可评估，全称见 §4.1）、I4 方法族被占
...
- **证据等级**：检索模板五级 = DISCOVERED / ABSTRACT_VERIFIED / FULLTEXT_OPENED / CLAIM_VERIFIED /
  REPRODUCED。ledger v2 行内枚举保留其重构口径，与五级对应关系如下——CLAIM_LOCATED_FULLTEXT 35
  （≈FULLTEXT_OPENED + 承重定位）/ ABSTRACT_ONLY 20（≈ABSTRACT_VERIFIED）/
  FULLTEXT_UNREACHABLE_THIS_ROUND 2 / SYNTHESIS_PENDING_REVIEW 5（综合行专用，无五级对应）。
  snippet 不升全文级；综合行不继承最强子级；分桶报告，禁求和。
...
- **第三轮复审**（收档 a06a498，RETURN_FOR_MAJOR_REVISION，续42 零抗辩接受）：五承重缺陷坐实
  （①census schema 缺口——「92 resolved」含 6 条无 canonical ID + 56 条版本未钉、全作者/内容
  哈希 0/94；②ledger schema 缺口——content hash/verified_by/验证状态字段缺、版本未钉、复合行
  等级混承，「35 全文」实含 5 行摘要级；③「43 条 discrepancy」实为非空字段计数、其中 11 条以
  "None" 开头；④11 条提取期丢弃无逐项明细——当时最危险的分母缺口〔后经 journal 恢复闭合，
  §3.3〕；⑤**δ_corr 构念替换**——选择重合推不出误差相关）；另有独立发现（复审 §2.2，五缺陷
  之外）：送审快照字段语义误导——证据锚与送审锚拆分为双锚〔已照方执行并入 P0-R8 机检项〕。
  我方四处旧抗辩**全部获裁〔半〕胜**
...
- **P0-R 工单计分（第三轮复审裁定，续42 接受）**：R1 CLOSED（附新 provenance 发现）/
  R4 CLOSED_STRUCTURE_ONLY / R2·R3·R5·R7 PARTIAL / R6 REOPENED（δ_corr）/ R8 NOT_DONE。
  其后进展（不改历史计分，以工件为准）：R2→census v2、R3/R7→ledger v2、R6→修正案 №1 生效、
  R8→校验器 v2 OVERALL PASS（详 §3.3–§3.6）；人类双审与独立盲重建（E2）未做。
...
- **版本 pin 收尾**（b594820）：16 行 PIN_PENDING 全部经 cluster→work 映射钉定（29 个
  claim×work pin，100%）。
- **独立验收抽样**：另一 reviewer 镜头抽 10/10 census + 17/17 MATERIAL/CRITICAL +
  7/7 摘要级承重裁决，**零同类错误 → ACCEPTANCE_PASS**（纪律：发现 1 个同类错即扩全量复筛）。
  **事故披露（双锚）**：该报告首次提交（b1af8c6）时为 **0 字节**——提交信息先于证据字节存在，
  属「提交信息不实」类事故；全文由协调者据会话内代理返回重建、带事故注与重跑要约入库
  （`docs/checks/2026-07-15-gate-a-acceptance-sampling.md`）——**证据字节锚 = e87bdf9**，
  b1af8c6 仅为事故记录锚；「独立抽样验收级」的证据分量应按重建件（非独立提交字节）折算。
...
- **合同 v1**（dce5c79，owner 治理签核，续41）：六份身份合同（I1 / bare-I2 / strict-I2 / I3 /
  I4 / UMBRELLA，每份=冻结定义+量词规则+正负测试+出处日期链；候选身份 strict-I2/I3/I4/UMBRELLA
  另带 kill/pivot 判据〔proceed-if 仅 I4 显式冻结〕，已占身份 I1/bare-I2 只登记占据现状）+
...
- **修正案 №1**（0a5e108 生效；owner 两次独立 AskUserQuestion 分栏签署，未混栏）：
  ...
  D) 诚信核查 C1/C4 终验与探针授权拆两个 exact-hash 签字块（根治循环签署）。随生效：探针协议
  v1 作废（v2 于 Gate C 重写）。
...
- **C1 尝试普查**：registry vs raw run 集合差——补登 E 盘运行树 + W4 outputs + MLflow
  ...
  C4 = **CENSUS_COMPLETE**；owner 于 Integrity gate 独立签栏终验（治理性签署，非审计签署；
  两项诚信核查就此正式关闭）。
...
- **PRESS 2015 六要素敌意预检**：裁定 PRESS_REVISE，7 项修复照单应用（HIGH-1 多语 mega-query
  拆分；HIGH-2 arXiv cat-filter 16 条 + IEEE site: 路由 3 条；MOD-3 十项 text-word 变体；
  MOD-4 音频接地供给查询；MINOR-5 G6 阻断前置；MINOR-6 ernez23a Crossref DOI 解析步；
  MINOR-7 查询卫生规则）——应用于 18056f1 + 协调者逐字亲验残留清扫（aaffe4c，5 处缺口兜齐）+
...
证据级封顶=摘要级（ledger 枚举 ABSTRACT_ONLY，对应模板 ABSTRACT_VERIFIED——映射见 §2）待双审。
...
| I3 约束/可弃权/Goodhart selector | abstain 分量已占；I3-combined 保留记录中无匹配 | Goodhart-on-speech 拐点是其生死检查点 |
...
LibriSpeech WER Beam 0.042 / MBR 0.033 / Oracle 0.013——MBR 兑现约 31% 头空，rho_greedy 锚
〔默认输出=beam〕）。杀伤在更正后
...
任务格覆盖单独报告为 MIXED/UNDERSEARCHED（合同 §2 原文「ST/SLU 格暂无同类」；SER 格则已被
jia-SER 占据）。
...
（CL2-0011，台账更正后如实降格）。conformal 侧：ernez23a（PMLR v204）占 conformal-ASR，其
「80%」系置信水平而非经验覆盖率（CL2-0009，我方引用已更正）。
...
《The Art of Scaling Test-Time Compute》2512.02008、RoboMonkey 2506.17811 VLA scaling law）。**但音频/omni 的供给分层实例化
...
SQA +14.42 BERTScore / SSUM +3.85 / ST +6.11 COMET；label-free 兑现（rho_greedy 锚）ASR 达
77.6%（Likelihood）/ 60.0%（Lik+MBR），ST 小幅为正（+0.93 / +1.09），**SQA/SSUM 为负**
（Likelihood −11.06 / −8.60；Lik+MBR −3.33 / −2.19）——「规律」可能主要是**何时失效**的规律，这是 I4 的核心素材而非尴尬。
...
READ（P-0075）Table 1 兑现率（rho_greedy 锚）7.7%–68.5%（ASRU 7.7%、SWBD 68.5%；LibriSpeech
仅 12–17%，CL2-0061）——我方此前「~70–85% oracle」引用已更正。
...
禁只引低端；CL2-0013）。其算子对象与池内选择已按修正案 §C 分离；等预算 loop vs 一次性 rerank 判别留
Stage-1C。

### 4.3 已知集外压力（诚实登记——最强压力恰在保留记录集之外）

三篇尚未入台账、已列 round-2 必查（L-NEW lanes；其数字均 AS_GIVEN_BY_REVIEW，未经我方对账）：
**2606.02981**（labeled BoN-gain predictor，Spearman ρ≈0.90 量级——逼迫 I4 差异化必须
label-free）；**2607.05391**（text-agent 上 verifier
...
round-2 后的否定性结论仍**按身份索引** + 记录集版本钉定 + 强制伴随 token（token 覆盖层
`2026-07-15-replayability-template-token-overlay.md` + RESP-02 §3.3；协议 §6 措辞已同步对齐
覆盖层），不得写裸 saturated/novel/complete。

### 6.3 Stage-1B 四探针（协议 v2 待 Gate C 重写签批；v1 已作废）

设计要点（已按第三轮复审科学修正更新）：**共享池纪律**——四探针共用 P-α 池（c0=裸 prompt，
K=16，单次触碰，失败保留在分母）；**格**：ASR 格（LibriSpeech **dev-other**——test-other 系
publication holdout 不触碰）/ SER 格（CREMA-D 子集）/ 音频理解格（mmau-mini），每格 n=60–100
（格代号留待探针协议 v2 登记后启用——v1 的 C-* 代号与 claim-ledger 的 C-〈主题〉ID 族冲名，
不再沿用）；模型 = 锁内 Qwen3-Omni-30B GGUF（llama-server 常驻）。

- **P-α 头空**：每格 (U_greedy, E[U_pool], U_oracle) → H(c)。**全格 H(c)≈0 → 停线上报 owner**
  （动摇整个 selector 纲领，不自行解释）。
- **P-β MBR 基线**：主臂 = 文献可比 BLEU 成对效用（1−WER 降为 sensitivity 臂）；LibriSpeech
  ~31%（rho_greedy 锚）只作 **external reference，不设复现预期**（复审 §9.3 更正，续42 接受）
  ——关注点是兑现率的供给/数据依赖方向（对照 ReazonSpeech ~9%）。
...
## 7. 可证伪承诺（判据来源分层标注；裁决人=owner，Stage-1C）
...
| I4 | ρ(c) 实测为噪声，或矩阵级无头空（H(c)≈0 across cells） | 无增量预测力 → 降为工程实例化（不得作科学新颖性）† | 曲面升级为相对通用 baseline 有**label-free 增量预测力**的规律（合同 §5 强制检查点） |
| UMBRELLA | 等预算下 loop ≤ one-shot BoN（合同 §6 原文；loop vs BoN/MBR/rerank 三比较系修正案 §C 的保留研究问题，非 kill 判据） | loop ≈ 一次性 rerank（只是 test-time compute） | 等预算下 loop 有真增量且 agent 实际接触音频 † |
| 全局停线 † | 探针全格 H(c)≈0 → 停线上报 owner——动摇整个 selector 纲领，不自行解释 | — | — |

**判据来源分层（如实标注）**：各行 kill-if 与 pivot-if = 合同 v1 + 修正案 №1 **冻结文本**
（strict-I2 两测试=修正案 §B；I4 proceed-if=合同 §5 冻结检查点）。带 † 者 =
决策包/探针协议沿革表述（探针协议 v1 已作废）——**非合同冻结文本**，须在探针协议 v2（Gate C）
重新冻结后方可作裁决判据；其中 I4 pivot-if 的「降级为工程实例化」后果在合同 §5 检查点文本中
有出处，† 系保守标注。

强制报告清单（每次 selector 运行，照录修正案 §B）：双 selector 各自 Recovery/PCS/rho/regret
（PCS 系修正案原文代号，待术语表登记正名）、rank correlation、2×2 同错四格计数、
selection_overlap（仅描述）、router 上界与 complementary_gain。
...
round-2 尚未发出任何查询；诚信核查 C1 的 config-selection 历史轨迹为已登记永久缺口；理论轨道
的收敛定理只覆盖 τ→0/预算约束的理想化前提，未触及供给条件化。本件按 P0-R8 状态门机检通过后
提交（运行工件 `docs/checks/2026-07-15-p0r8-gate-run-proposal-commit.txt`）。

## 9. 对 reviewer 的具体请求

1. **round-2 协议 v2 的 search-design 签署**（协议 §14 执行 preflight 阻断项——Gate B 六门
   之外的执行前置）：对 21 lanes / 105 条
...
| census v2 + ledger v2 | 28ad858；版本 pin 收尾 b594820；验收抽样：事故记录锚 b1af8c6（0 字节）/ 报告重建字节锚 e87bdf9（§3.3 事故披露） |
