...
| 2026-07-14（修正案 №1，已生效——owner 两栏分签） | strict-I2 / UMBRELLA / same-selector | δ_corr 拆名（选择重合移出语义）+ strict-I2 kill-if 重写为两独立测试（音频接地 matched-controls 版 / 同错×无互补）+ §7 对 UMBRELLA 覆盖撤回 | 第三轮复审 §6.3/9.2（构念替换裁定） | 否（判据可执行化 + 合同适用范围更正，不动任何身份定义与占据裁决） | 协调者 |
