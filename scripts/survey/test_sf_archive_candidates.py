...
class ArchiveRepoFixture(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.write("wiki/survey/current/protocol.md", b"current protocol\n")
    ...
    @staticmethod
    def manifest_bytes(paths: list[str]) -> bytes:
        return (
            json.dumps(
                {
...
class ArchivePreMoveTests(ArchiveRepoFixture):
    ...
    def test_candidate_in_registry_or_current_manifest_fails_without_writing(self) -> None:
        ...
        cases = (
            ...
            (
                "archive-source-current-manifest",
                "wiki/survey/current/manifest.json",
                self.manifest_bytes([self.source]),
            ),
        )
    ...
    def test_staged_current_manifest_is_trusted_when_worktree_matches_index(self) -> None:
        self.write(
            "wiki/survey/current/manifest.json",
            self.manifest_bytes(["wiki/survey/current/protocol.md"]),
        )
        git(self.repo, "add", "--", "wiki/survey/current/manifest.json")
