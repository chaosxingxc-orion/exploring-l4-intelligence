...
def render_opening_table(
    ...
    if not isinstance(method_paths, int) or not isinstance(unique_works, int):
    ...
    try:
        ...
        report = json.loads(
            ...
            parse_constant=lambda token: (_ for _ in ()).throw(
                CurrentTableError(f"non-finite JSON constant: {token}")
            ),
        )
...
def main(argv=None) -> int:
    ...
    except (CurrentTableError, OSError) as error:
