#!/usr/bin/env python3
"""Fail-closed pre-adjudication agreement for Agentic calibration R2R1."""
...
if __package__ in {None, ""}:
    ...
    import sf_stage1c_v2_calibration_agreement_v6 as v6
    ...
    from scripts.survey import sf_stage1c_v2_calibration_agreement_v6 as v6
...
# Patched to the exact deterministic R2R1 package before method-review commit.
FROZEN_CONTRACT_SHA256 = "a3ec30918d12cd4825a2bbd022316b0149045f35dc6b791f0bcc75da0f17cedd"
AGREEMENT_MINIMUM = 0.85

OBJECT_ARRAYS = r2r1.OBJECT_ARRAYS
...
CRITICAL_OBJECT_FIELDS: dict[str, tuple[str, ...]] = {
    **{
        key: value
        for key, value in v6.CRITICAL_OBJECT_FIELDS.items()
        if key not in {"translation_or_compatibility_decisions", "paper_reproduction_support"}
    },
    "translation_or_compatibility_decisions": (
        *v6.CRITICAL_OBJECT_FIELDS["translation_or_compatibility_decisions"],
        "target_object_type",
    ),
    "paper_reproduction_support": (
        ...
        *(f"field_evidence_states.{field}" for field in r2r1.REPRODUCTION_FACT_FIELDS),
        "source_locator_ids", "closure_status", "blockers",
    ),
}


class AgreementError(ValueError):
    """Raised before metrics when R2R1 provenance or intake is invalid."""


def _artifact_sha(value: object) -> str:
    return r2r1.sha256_bytes(r2r1.json_bytes(value))
...
def _validate_static_artifacts(
    *, frozen: dict[str, Any], runtime_intake: dict[str, Any],
    ...
    submission_receipt_schema: dict[str, Any], positive_support_preflight: dict[str, Any],
) -> None:
    if _artifact_sha(frozen) != FROZEN_CONTRACT_SHA256:
        raise AgreementError("caller input differs from compiled R2R1 frozen contract")
    ...
    exact = (
        ("response schema", response_schema, "response_schema_sha256"),
        ("source manifest", source_manifest, "source_manifest_sha256"),
        ("distribution manifest", distribution_manifest, "distribution_manifest_sha256"),
        ("delivery receipt schema", delivery_receipt_schema, "delivery_receipt_schema_sha256"),
        ("submission receipt schema", submission_receipt_schema, "submission_receipt_schema_sha256"),
        ("positive support preflight", positive_support_preflight, "positive_support_preflight_sha256"),
    )
    for label, artifact, key in exact:
        if _artifact_sha(artifact) != frozen.get(key):
            raise AgreementError(f"{label} differs from frozen R2R1 package")
    try:
        projection = r2r1.static_intake_projection(runtime_intake)
    except r2r1.ContractError as error:
        raise AgreementError(str(error)) from error
    if _artifact_sha(projection) != frozen.get("base_intake_static_sha256"):
        raise AgreementError("runtime agreement differs from frozen R2R1 base intake")
    if _artifact_sha(r2r1.r2._rendition_map(source_manifest)) != frozen.get("paper_rendition_map_sha256"):
        raise AgreementError("source manifest differs from frozen R2R1 rendition map")
    if distribution_manifest.get("content_bundle_sha256") != frozen.get("content_bundle_sha256"):
        raise AgreementError("distribution bundle differs from frozen R2R1 package")
    if distribution_manifest.get("coder_prompt_sha256") != frozen.get("prompt_sha256"):
        raise AgreementError("distribution prompt differs from frozen R2R1 package")
...
def _validate_runtime_intake(intake: dict[str, Any]) -> tuple[list[str], list[dict[str, Any]]]:
    if intake.get("schema") != "sf-stage1c-v2-agreement-intake-contract-v7":
        raise AgreementError("unknown R2R1 runtime agreement intake schema")
    ...
    for flag in (
        ...
        "typed_local_object_maps_required",
        "affirmative_reproduction_closure_required",
        "exact_response_bytes_bound_before_agreement",
    ):
        if not intake.get(flag):
            raise AgreementError(f"runtime intake lacks required flag: {flag}")
    ...
    for slot in slots:
        ...
        if slot.get("planned_model") != expected_model or slot.get("model") != expected_model:
            raise AgreementError("bound coder model differs from isolated-model plan")
        for field in (
            "submission_receipt_id", "submission_receipt_sha256", "response_sha256",
            "response_bytes", "response_count",
        ):
            if slot.get(field) in {None, ""}:
                raise AgreementError(f"coder slot lacks frozen response binding: {field}")
...
def _validate_delivery_receipts(
    *, receipts: list[dict[str, Any]], slots: list[dict[str, Any]],
    ...
    try:
        v6._validate_delivery_receipts(
            receipts=receipts, slots=slots, schema=schema,
            distribution=distribution, intake=intake,
        )
    except v6.AgreementError as error:
        raise AgreementError(str(error)) from error


def _root_from_receipts(
    frozen: dict[str, Any], receipts: list[dict[str, Any]]
) -> dict[str, Any]:
    rows = [
        {
        ...
        for receipt in sorted(receipts, key=lambda row: row["coder_slot"])
    ]
    root = {
        "schema": "sf-stage1c-v2-frozen-response-root-v1",
        "status": "FROZEN_RESPONSES_BOUND",
        "frozen_package_contract_sha256": _artifact_sha(frozen),
        "submission_receipts": rows,
    }
    root["root_sha256"] = _artifact_sha(root)
    return root


def _validate_submission_receipts_and_bytes(
    *, receipts: list[dict[str, Any]], raw_by_slot: dict[str, bytes],
    slots: list[dict[str, Any]], schema: dict[str, Any], intake: dict[str, Any],
    frozen: dict[str, Any], paper_ids: list[str],
) -> dict[str, list[dict[str, Any]]]:
    if not isinstance(raw_by_slot, dict) or set(raw_by_slot) != {"A", "B"}:
        raise AgreementError("exact frozen response bytes are required for slots A/B")
    if len(receipts) != 2 or {row.get("coder_slot") for row in receipts} != {"A", "B"}:
        raise AgreementError("submission receipts require exact coder slots A/B")
    receipt_by_slot = {row["coder_slot"]: row for row in receipts}
    parsed: dict[str, list[dict[str, Any]]] = {}
    for slot in slots:
        name = slot["coder_slot"]
        receipt = receipt_by_slot[name]
        try:
            Draft202012Validator(schema).validate(receipt)
        except ValidationError as error:
            raise AgreementError(f"invalid submission receipt: {error.message}") from error
        expected_receipt_sha = _artifact_sha(r2r1._submission_receipt_projection(receipt))
        if receipt.get("receipt_sha256") != expected_receipt_sha:
            raise AgreementError("submission receipt hash differs")
        for field in (
            "coder_id", "coder_transaction_id", "process_id", "task_id", "model",
        ):
            if receipt.get(field) != slot.get(field):
                raise AgreementError(f"submission receipt differs from runtime slot: {field}")
        if (
            receipt["receipt_id"] != slot["submission_receipt_id"]
            or receipt["receipt_sha256"] != slot["submission_receipt_sha256"]
            or receipt["response_sha256"] != slot["response_sha256"]
            or receipt["response_bytes"] != slot["response_bytes"]
            or receipt["response_count"] != slot["response_count"]
        ):
            raise AgreementError("submission receipt differs from frozen runtime intake")
        raw = raw_by_slot[name]
        if (
            not isinstance(raw, bytes)
            or len(raw) != receipt["response_bytes"]
            or r2r1.sha256_bytes(raw) != receipt["response_sha256"]
        ):
            raise AgreementError("frozen response bytes differ from submission receipt")
        try:
            rows = r2r1._decode_canonical_responses(raw)
        except r2r1.ContractError as error:
            raise AgreementError(f"invalid frozen response bytes: {error}") from error
        if [row.get("paper_id") for row in rows] != paper_ids:
            raise AgreementError("frozen response bytes differ from canonical paper order")
        if _artifact_sha(paper_ids) != receipt["canonical_paper_ids_sha256"]:
            raise AgreementError("submission receipt paper-ID digest differs")
        parsed[name] = rows
    expected_root = _root_from_receipts(frozen, receipts)
    if intake.get("frozen_response_root") != expected_root:
        raise AgreementError("frozen response root differs from exact submission receipts")
    return parsed
...
def _validate_response_set(
    ...
    for paper_id, response in indexed.items():
        ...
        try:
            r2r1.validate_completed_response(response, response_schema, source_manifest)
            compiled[paper_id] = r2r1.compile_response_objects(response, source_manifest)
        except (r2r1.ContractError, ValueError) as error:
...
def _compute_metrics(
    ...
    for field in SINGLE_LABEL_FIELDS:
        paper_level[field] = v3._paper_field_gate(
            pairs, field, minimum=minimum, multilabel=False
        )
    for field in MULTILABEL_FIELDS:
        paper_level[field] = v3._paper_field_gate(
            pairs, field, minimum=minimum, multilabel=True
        )
    ...
    for array_name in OBJECT_ARRAYS:
        ...
        statuses = [segmentation_status, *(row["gate_status"] for row in critical_gates.values())]
...
def compute_agreement(
    response_bytes_by_slot: dict[str, bytes], *, runtime_intake: dict[str, Any],
    frozen_contract: dict[str, Any], response_schema: dict[str, Any],
    source_manifest: dict[str, Any], distribution_manifest: dict[str, Any],
    delivery_receipt_schema: dict[str, Any], delivery_receipts: list[dict[str, Any]],
    submission_receipt_schema: dict[str, Any], submission_receipts: list[dict[str, Any]],
    positive_support_preflight: dict[str, Any], minimum: float = AGREEMENT_MINIMUM,
) -> dict[str, Any]:
    ...
    _validate_static_artifacts(
        ...
        submission_receipt_schema=submission_receipt_schema,
    ...
    parsed = _validate_submission_receipts_and_bytes(
        receipts=submission_receipts,
        raw_by_slot=response_bytes_by_slot,
        slots=slots,
    ...
    packet_by_paper = {
        row["paper_id"]: row["packet_item_id"] for row in runtime_intake["items"]
    }
    expected_ids = set(paper_ids)
    left = _validate_response_set(
        parsed["A"], slot=slots[0], expected_ids=expected_ids,
        packet_by_paper=packet_by_paper, intake=runtime_intake,
        response_schema=response_schema, source_manifest=source_manifest,
    )
    right = _validate_response_set(
        parsed["B"], slot=slots[1], expected_ids=expected_ids,
        packet_by_paper=packet_by_paper, intake=runtime_intake,
        response_schema=response_schema, source_manifest=source_manifest,
    )
    metrics = _compute_metrics(left, right, paper_ids, AGREEMENT_MINIMUM)
    return {
        "schema": "sf-stage1c-v2-calibration-agreement-result-v7",
        ...
        **metrics,
        ...
        "submission_receipts_validated": True,
        "frozen_response_root_validated": True,
...
if __name__ == "__main__":
    raise SystemExit("Import compute_agreement with two exact frozen R2R1 response byte arrays.")
