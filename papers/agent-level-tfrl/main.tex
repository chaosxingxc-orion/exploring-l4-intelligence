...
\paragraph{One concrete mechanism that could win---and the ablation that would isolate it.} A testbed is only honest if it states what a \emph{win} would look like and how it would be attributed. We commit to one falsifiable candidate. The strong baseline---single-enrollment ECAPA-TDNN with an SER head---commits to a \emph{single} speaker template per identity and reads each turn against it. Its documented weakness is \emph{genuine channel variation}: a speaker recorded across different devices, rooms, codecs, and health/affect states drifts away from a one-shot enrollment centroid. The candidate mechanism is \textbf{Bayesian multi-session speaker-evidence integration}: instead of one template, the agent's memory accumulates a \emph{posterior} over a speaker's embedding from many sessions, integrating evidence under an explicit channel-variation model, so that identity is decided by the accumulated multi-session posterior rather than by proximity to a single enrollment vector. If cross-session integration helps anywhere, it helps precisely where single-enrollment ECAPA is structurally blind. To \emph{isolate} this mechanism from mere encoder quality, the pre-registered ablation holds the front end fixed and varies only the memory representation: (i) a \emph{curated multi-session graph} (the proposed posterior-integrating store) versus (ii) a \emph{running-centroid dictionary} (a single moving-average template per speaker), \emph{same speaker encoder for both}. A win is a strict improvement of (i) over (ii) on cross-session, cross-channel speaker verification/identification with matched compute. We are honest that this is a \emph{hypothesis to be tested}, not a result: absent a positive (i)-over-(ii) ablation, the system reduces to \textbf{memory curation over the very same encoder the baseline already reads}, and offers no paralinguistic capability the baseline lacks. That reduction---and the negative result it implies---is an acceptable outcome for a testbed; what would be unacceptable is to assert the win without the ablation.
...
\paragraph{Primary baseline: a classic speech-encoder pipeline the agent must exceed.} The single most important addition to this plan is a \emph{teethed} primary baseline with a \emph{pre-registered, locked} accumulation rule. We pre-register, in the locked release artifact, the baseline's \emph{exact} accumulate / \textsc{update} / \textsc{delete} rule, justified as the strongest \emph{simple} pipeline: (a) a per-speaker running \textbf{ECAPA-TDNN centroid} \citep{ecapatdnn2020,xvector2021} with $k$-shot enrollment---the speaker key is the mean of that speaker's $k$ most recent enrolled embeddings; (b) per-speaker SER state maintained by \textbf{majority vote with exponential decay} over per-session affect predictions; and (c) an explicit \textbf{change-detection threshold} on centroid/affect drift that decides \textsc{update} (refresh the running key) versus \textsc{delete}-and-re-enroll. We require the self-evolving agent to \emph{strictly exceed} this pipeline, with the contrast tested by a paired bootstrap on per-item deltas \citep{bisani2004bootstrap}. We pre-register the falsifier explicitly: \textbf{if the agent does not exceed the ECAPA$+$SER pipeline with a paired-bootstrap CI excluding $0$, this is recorded as \emph{not supporting the agentic claim} on paralinguistics}, and reported as such. The baseline is deliberately strong precisely because that one frozen content bi-encoder is at chance on speaker: the classic pipeline encodes exactly the paralinguistic competence the omni \emph{content} embedding lacks, so beating it is the real test of whether the \emph{agentic} composition (memory $\times$ policy $\times$ verifiable gate) adds anything beyond a simple, fixed per-speaker dictionary. The baseline, with its locked update rule, is shipped in the release artifact so any reader can re-run the comparison.
...
M2 & 1 & Cross-session benchmark $+$ ECAPA/SER baseline & genuine variation; DER/SV-EER/SER in-band; locked update rule shipped \\
...
M5 & 2 & Compounding vs.\ baseline (evidence-matched) & slope exceeds classic baseline under matched per-speaker evidence (paired CI) \emph{or} documented null \\
...
\hline
\end{tabular}
\caption{Value-of-information--ordered milestones. Each gate is a verifiable, bootstrap-tested criterion; the locked test set is touched once per phase, after dev-only selection (no oracle test-layer selection), with FWER correction across the sweep. The compounding gate (M5) requires \emph{exceeding the classic ECAPA$+$SER baseline under matched per-speaker evidence}, not merely a positive slope.}
\label{tab:milestones}
\end{table}

The release artifact bundles the benchmark builder (including the realism controls: cross-session variation requirement, numeric DER/SV-EER/SER admission bands, listen-vs-read filtering on own ASR transcripts with surface-marked and normalized variants, the spontaneous-affect split, and the SER human-annotation pipeline), the ECAPA$+$SER baseline with its pre-registered locked update rule (per-speaker running ECAPA centroid with $k$-shot enrollment, SER majority-with-decay, explicit change-detection threshold), the raw-embeddings-only memory schema, the loop configuration, pinned manifests, the contamination-screen scripts, the committed paired-delta artifacts, and per-milestone \texttt{reproduce:} commands, so that all of \Cref{tab:milestones} can be re-derived end-to-end on equivalent hardware. This ordering guarantees that the expensive compounding study (M4--M7) is only attempted after the instrument and its baseline (M2--M3) and the gate and spread map (M0--M1) are independently certified---including their artifact, contamination, and selection-leak controls---maximizing the information returned for the fixed single-GPU budget, and ensuring that an honest negative on the paralinguistic agentic claim is a publishable result of this plan rather than a hidden failure.
